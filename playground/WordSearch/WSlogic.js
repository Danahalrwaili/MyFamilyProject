...
function WordSearchLogic(gameId,list) {
	...
	var board = {
		...
		size: 5 //width + height of the matrix
	...
	this.setUpGame = function() {
		...
		fillWithRandomLetters(board.matrix, list);
	...
	function fillWithRandomLetters(matrix, list) {

    let lettersPool = "";

    // نجمع كل الحروف من كلمات الثيم
    for (let i = 0; i < list.length; i++) {
        for (let j = 0; j < list[i].length; j++) {
            lettersPool += list[i][j];
        }
    }

    // إزالة المسافات
    lettersPool = lettersPool.replace(/\s+/g, "");

    for (let i = 0; i < matrix.length; i++) {
        for (let j = 0; j < matrix[i].length; j++) {
            if (jQuery.isEmptyObject(matrix[i][j])) {
                let randIndex = Math.floor(Math.random() * lettersPool.length);
                matrix[i][j] = lettersPool.charAt(randIndex);
            }
        }
    }
}
