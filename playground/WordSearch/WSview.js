...
function WordSearchView(matrix, list, gameId, listId, wordsFound) {
    ...
    */
    ...
    function normalizeArabic(word) {
        return word
            .replace(/[^ء-ي]/g, "") // يبقي فقط الحروف العربية
            .replace(/[أإآ]/g, "ا"); // توحيد الألف
    }

    var normalizedCheck = normalizeArabic(wordToCheck);

    for (var i = 0; i < list.length; i++) {
        for (var j = 0; j < list[i].length; j++) {
            var normalizedWord = normalizeArabic(list[i][j]);

            // إذا الكلمة صحيحة أو عكسها
            if (normalizedCheck === normalizedWord || normalizedCheck === reversedWord(normalizedWord)) {

                // أضفها للمصفوفة wordsFound
                wordsFound.push(normalizedWord);

				// شغّل صوت correct
                document.getElementById("correctSound").play();


                // شطب الكلمة من قائمة الثيم (تغيير اللون/اضافة class)
                $(".listWord").filter(function() {
                    return normalizeArabic($(this).text()) === normalizedWord;
                }).addClass(selfSolved ? "User" : "found");

                // تحقق إذا كل الكلمات تم إيجادها
